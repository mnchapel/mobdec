...
public:
    ...
    // constructor MN (put two CTensor in one CTensor)
    CTensor(const CTensor<T>& tensor1, const CTensor<T>& tensor2);
    // destructor
    virtual ~CTensor();
...
// constructor MN ----------------------------------------------------------------------------
template <class T>
CTensor<T>::CTensor(const CTensor<T>& tensor1, const CTensor<T>& tensor2)
{
    if(tensor1.mXSize != tensor2.mXSize
    || tensor1.mYSize != tensor2.mYSize
    || tensor1.mZSize != tensor2.mZSize)
        std::cout << "Error : the two tensors must be the same image\n";

    mXSize = tensor1.mXSize;// + tensor2.mXSize;
    mYSize = tensor1.mYSize + tensor2.mYSize;
    mZSize = tensor1.mZSize;

    mData = new T[mXSize*mYSize*mZSize];

    if (mZSize == 1)
    {
        const unsigned int tensor1_size = tensor1.mXSize*tensor1.mYSize;
        for (int i=0; i<tensor1_size; i++)
            mData[i] = tensor1.mData[i];

        const unsigned int tensor2_size = tensor2.mXSize*tensor2.mYSize;
        for (int i=0; i<tensor2_size; i++)
            mData[i+tensor1_size] = tensor2.mData[i];
    }

    else
    {
        int aSize = mXSize*mYSize;
        int aSizeTwice = aSize*2;

        const unsigned int tensor1_size = tensor1.mXSize*tensor1.mYSize;
        for(int i=0; i<tensor1_size; i++)
        {
            mData[i] = tensor1.mData[i];
            mData[i+aSize] = tensor1.mData[i+tensor1_size];
            mData[i+aSizeTwice] = tensor1.mData[i+tensor1_size*2];
        }

        const unsigned int tensor2_size = tensor2.mXSize*tensor2.mYSize;
        for(int i=0; i<tensor2_size; i++)
        {
            mData[i+tensor2_size] = tensor2.mData[i];
            mData[i+aSize+tensor2_size] = tensor2.mData[i+tensor2_size];
            mData[i+aSizeTwice+tensor2_size] = tensor2.mData[i+tensor2_size*2];
        }
    }
}
